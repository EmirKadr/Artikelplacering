...
def test_check_for_update_returns_none_for_current_version():
    ...
    assert check_for_update(current_version="0.1.0", session=FakeSession(response)) is None
...
def test_check_for_update_finds_setup_asset():
    response = FakeResponse({
        ...
        "assets": [
            {"name": "Artikelplacering-0.2.0-win64.zip", "browser_download_url": "zip"},
            {"name": "Artikelplacering-0.2.0-Setup.exe", "browser_download_url": "exe"},
        ],
    })
    ...
    assert info.installer_name == "Artikelplacering-0.2.0-Setup.exe"
...
def test_download_update_installer_writes_file(tmp_path):
    ...
    info = UpdateInfo(
        ...
        installer_name="Artikelplacering-0.2.0-Setup.exe",
    )
    progress = []
    ...
    assert path.name == "Artikelplacering-0.2.0-Setup.exe"
